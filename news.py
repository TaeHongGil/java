def solution(str1, str2):
    str1 = [str1[i-2:i].lower() for i in range(2,len(str1)+1) if str1[i-2:i].isalpha()]
    str2 = [str2[i-2:i].lower() for i in range(2,len(str2)+1) if str2[i-2:i].isalpha()]
...
입력으로 들어온 문자열은 두 글자씩 끊어서 다중집합의 원소로 만든다. 이
때 영문자로 된 글자 쌍만 유효하고, 기타 공백이나 숫자, 특수 문자가 들어있는 경우는 그 글자 쌍을 버린다. 예를 들어 ab+가 입력으로 들어오면, ab만 다중집합의 원소로 삼고, b+는 버린다.
