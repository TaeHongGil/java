...
def solution(s):
    ...
    for i in range(1,round(len(s)/2)+1):
        ...
        for i in range(0,answer.count("1")):
            answer.remove("1")

        a.append(len("".join(answer)))
...
2개 단위로 잘라서 압축한다면 2ab2cd2ab2cd로 표현할 수 있습니다. 다른 방법으로 8개 단위로 잘라서 압축한다면 2ababcdcd로 표현할 수 있으며, 이때가 가장 짧게 압축하여 표현할 수 있는 방법입니다.
