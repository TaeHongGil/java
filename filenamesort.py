...
def solution(files):
    ...
    for i in range(len(files)):
        files[i]=files[i],re.findall("\d+",files[i]),re.findall("\D+",files[i])
    for i in range(len(files)):
        files[i][1][0]=files[i][1][0].zfill(4)

    files=sorted(files,key=lambda x:(str(x[2][0]).lower(),int(x[1][0])))
...
파일명은 영문자로 시작하며, 숫자를 하나 이상 포함하고 있다.
중복된 파일명은 없으나, 
대소문자나 숫자 앞부분의 0 차이가 있는 경우는 함께 주어질 수 있다. (muzi1.txt, MUZI1.txt, muzi001.txt, muzi1.TXT는 함께 입력으로 주어질 수 있다.)
출력 형식
